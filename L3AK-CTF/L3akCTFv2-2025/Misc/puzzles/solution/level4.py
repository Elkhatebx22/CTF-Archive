...
s.cookies.set("session", "sOnOQmj0irS82bBbqu_XQ1HvgE7HO_Mhs8ptbMnyc-dDCdb1FnEh2Oup37sMYo3mwVNz92E0aZnqOF-xMcCRgcLIVVLrEHzR4CKC5IW4K4dY2R0yn_X-M-lHXlAk-lKlIT9cDbWjbCSXZBVYEJrymRe1hlidVbAmw5XPSjCb82w=")
...
while i < 1:
    ...
    try:
        ...
        print(puzzle.name, puzzle.piece_height, puzzle.piece_width)
        order = puzzle.solve().order()
        time.sleep(max(0, 0.350-time.time()+s_t))

        r = s.post(ENDPOINT+'/api/checkanswer', json={'puzzle_id': puzzle.puzzle_id, 'answer': order}).json()
        ...
        if r['correct'] == False:
            ...
            puzzle.make_image().save(f"wrong_{puzzle.name}_{attempt}.png")
            attempt += 1
            if attempt > 2:
