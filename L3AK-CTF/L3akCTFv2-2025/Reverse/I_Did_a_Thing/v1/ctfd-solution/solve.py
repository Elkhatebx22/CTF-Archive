...
rc4_src= """function WxdMAKdC(keyBuf,dataBuf){const S=Array(256);for(let i=0;i<256;i++){S[i]=i;}let j=0;for(let i=0;i<256;i++){j=j+S[i]+keyBuf[i%keyBuf.length]&0xff;let temp=S[i];S[i]=S[j];S[j]=temp;}const out=Buffer.alloc(dataBuf.length);let i=0;for(let n=0;n<dataBuf.length;n++){i=i+1&0xff;j=j+S[i]&0xff;let temp=S[i];S[i]=S[j];S[j]=temp;out[n]=dataBuf[n]^S[S[i]+S[j]&0xff];}return out;}"""

interp_src="""function ZggluiPo(){let AGxzLWil={};let iVMWmKNV=[],CXuPQbzt=[{}],oMrqglkM=[],ifiCWOuo=0;Object.defineProperty(CXuPQbzt,AGxzLWil,{get(){return AGxzLWil;},set(v){AGxzLWil=v;},enumerable:true,configurable:true});let HteDywqV=null,XQrvlkXC=0,QInPVZUD=582442495;let KSDQeeAz=[];for(let OGahjnQM=0;OGahjnQM<YTEgdBrw.length;OGahjnQM++){KSDQeeAz[OGahjnQM]=Buffer.from(YTEgdBrw[OGahjnQM],"base64");}const cOZXAhuR=iGNBQOOq=>iVMWmKNV.push(iGNBQOOq);const hZkqrYUP=()=>iVMWmKNV.length?iVMWmKNV.pop():(()=>{throw new Error("ball");})();let oXQnMxgQ=[];AGxzLWil["wODpUeYg"]=ztNXSAcf=>{iVMWmKNV.push(ztNXSAcf.uKbkIKDv);XQrvlkXC++;};AGxzLWil["vQbjpVuO"]=ztNXSAcf=>{const qqvWrevR=hZkqrYUP(),xPQPzcsA=hZkqrYUP(),dryLQyoz=hZkqrYUP();const xjVKxVWS=WxdMAKdC(Buffer.from(xPQPzcsA,"base64"),Buffer.from(qqvWrevR,"base64")).toString();AGxzLWil[dryLQyoz]=VWbfeqCl('('+xjVKxVWS+')');XQrvlkXC++;};const VWbfeqCl=function(code){const f=function(...args){let wrapped=code;for(let i=CXuPQbzt.length-1;i>=0;i--){wrapped='with (CXuPQbzt['+i+']) { '+wrapped+' }';}return eval(wrapped)(...args);};return f;};while(XQrvlkXC<KSDQeeAz.length){const CkbOhaFE=Buffer.from(KSDQeeAz[XQrvlkXC],"base64");const YAsqkSEC=Buffer.alloc(CkbOhaFE.length);const rwwQknmA=[QInPVZUD&0xff,QInPVZUD>>>8&0xff,QInPVZUD>>>16&0xff,QInPVZUD>>>24&0xff];for(let OGahjnQM=0;OGahjnQM<CkbOhaFE.length;OGahjnQM++){YAsqkSEC[OGahjnQM]=CkbOhaFE[OGahjnQM]^rwwQknmA[OGahjnQM%4];}let glTDONCX=0;const xuMaXPfh=YAsqkSEC.readUInt8(glTDONCX++);const XclgXNTd=YAsqkSEC.toString("utf8",glTDONCX,glTDONCX+xuMaXPfh);glTDONCX+=xuMaXPfh;const DPOzHXaZ=YAsqkSEC.readUInt8(glTDONCX++);let uKbkIKDv;if(DPOzHXaZ===0){uKbkIKDv=YAsqkSEC.readDoubleLE(glTDONCX);glTDONCX+=8;}else{const sHugUSiZ=YAsqkSEC.readUInt16LE(glTDONCX);glTDONCX+=2;uKbkIKDv=YAsqkSEC.toString("utf8",glTDONCX,glTDONCX+sHugUSiZ);glTDONCX+=sHugUSiZ;}let ccXRrrXP;const qIylbpvY=YAsqkSEC.readUInt8(glTDONCX++);if(qIylbpvY){const PNIAJCGv=YAsqkSEC.readUInt8(glTDONCX++);if(PNIAJCGv===0){ccXRrrXP=YAsqkSEC.readDoubleLE(glTDONCX);glTDONCX+=8;}else{const JRiEbopc=YAsqkSEC.readUInt16LE(glTDONCX);glTDONCX+=2;ccXRrrXP=YAsqkSEC.toString("utf8",glTDONCX,glTDONCX+JRiEbopc);glTDONCX+=JRiEbopc;}}const eJbopdhI=YAsqkSEC.readUInt32LE(glTDONCX);glTDONCX+=4;const ZgtkqQSk=Math.imul(QInPVZUD,1664525)+1013904223>>>0;QInPVZUD=(ZgtkqQSk^eJbopdhI)>>>0;const xyRAQpoj=AGxzLWil[XclgXNTd];if(!xyRAQpoj)throw new Error("ball");xyRAQpoj({uKbkIKDv,ccXRrrXP});}return iVMWmKNV.length?hZkqrYUP():undefined;}"""
