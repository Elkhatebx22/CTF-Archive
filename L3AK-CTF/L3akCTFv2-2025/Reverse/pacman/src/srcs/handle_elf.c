...
void	handle_elf64(struct s_elf64 *elf)
{
	...
	prepare_pac(elf);
	if ((fd = open("pac", O_CREAT | O_RDWR | O_TRUNC, 0755)) <= 0)
		exit_error(ERR_UNKNOW);
	write(fd, (char*)elf->ptr, elf->size);
	close(fd);
}

void	init_elf64(struct s_file *file, struct s_elf64 *elf, uint64_t len)
{
	elf->ptr = memcpy(elf->ptr, file->ptr, file->size);
	elf->ptr = memset(elf->ptr + file->size, 0, len) - file->size;
...
struct s_elf64	*create_elf64(struct s_file *file)
{
	...
	if (!(elf = malloc(sizeof(struct s_elf64)))
	|| !(elf->ptr = malloc(file->size + len)))
		exit_error(ERR_UNKNOW);
	init_elf64(file, elf, len);
	return (elf);
}
