from flask import Flask, request, jsonify, session, render_template, redirect, url_for,abort,Response
...
def login():
    if request.method == "POST":
        ...
        if users.get(username) == password:
            ...
            if username == "admin":
                res = Response()
                res.set_cookie("token",flag,httponly=True, samesite="lax",path="/")
                return res
